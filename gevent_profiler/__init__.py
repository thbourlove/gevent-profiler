        ...
        if self.full_class:
            ...
            for cls in inspect.getmro(self.full_class):
                if self.co_name in cls.__dict__:
                    ...
                    if hasattr(fnc, "func_code")\
                            and fnc.func_code.co_filename == self.filename\
                            and fnc.func_code.co_firstlineno == self.line_no:
                        true_class = cls
...
def _print_output(duration):
    ...
    col_names = ["Call Name", "Count", "Cumulative", "Own Cumul",
                 "Child Cumul", "Per Call", "Own/Total"]
    ...
    for _, c in call_list:
        ...
        col_data = [c.name, "%d" % c.count, "%12f" % cumulative,
                    "%12f" % own_cumulative, "%12f" % children_cumulative,
                    "%12f" % per_call, own_ratio]
    ...
    # max widths
    widths = [max([len(row[x]) for row in output])
              for x in xrange(len(output[0]))]
    # build row strings
    fmt_out = [" ".join([x.ljust(widths[i]) for i, x in enumerate(row)])
               for row in output]
    # insert col separation row
    fmt_out.insert(1, " ".join([''.ljust(widths[i], '=')
...
def attach(duration=0):
    ...
    Tracing will stop automatically in 'duration' seconds. If duration is zero,
    the trace won't stop until detach is called.
    """
